...
class Analysis():
    ...
    def create_graph(self, params):
        try:
            ...
            latest_record = Thresholds.objects.latest('id')
            ...
            for plot_id in all_asset_in_plot:
                ...
                if not petaks.loc[petaks['plot_id'] == plot_id].empty:
                    ...
                    # plt.show()
                    plt.savefig(f"{filepath}/vi_{plot_id}_adjust_{adjust}.png")
                    # plt.savefig(f""+self._ANALYSIS_DIR_PATH+"vi_{plot_id}_adjust_{adjust}.png")
                    plt.close()
                    print('Plot Drawn Successfully')

                    #   place here Weed alert Code
            weed_alert = WeedAlert()
            weed_alert.generate_weed_alerts(params)
            return True
